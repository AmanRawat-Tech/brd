...
def bell_curve_meters_schedule_smooth_practical(sub_mru_df, holiday_df, start_date, end_date,
    ...
    for _, row in sub_mru_df.iterrows():
        ...
        smooth_installers = [initial_installers_subdiv]
...
def plot_monthly_meters_installed(schedule_df):
    df = schedule_df[schedule_df['Total_Meters'] > 10].copy()
    df['Year'] = df['Date'].dt.year
    df['Month'] = df['Date'].dt.month
    summary_month = (
        df.groupby(['Year', 'Month'])
        .agg(Meters_Installed=('Meters_Installed_Today', 'sum'))
        .reset_index()
    )
    summary_month['Year_Month'] = summary_month['Year'].astype(str) + '-' + summary_month['Month'].astype(str).str.zfill(2)
    summary_month = summary_month.sort_values(['Year', 'Month'])
    
    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=summary_month['Year_Month'],
        y=summary_month['Meters_Installed'],
        marker_color='skyblue',
        text=summary_month['Meters_Installed'].astype(int),
        textposition='auto'
    ))
    
    fig.update_layout(
        title=dict(text='Monthly Meters Installed (All Subdivisions)', x=0.5, xanchor='center'),
        xaxis_title='Year-Month',
        yaxis_title='Meters Installed',
        template='plotly_white',
        height=600,
        margin=dict(t=80, b=50, l=50, r=50),
        xaxis=dict(tickangle=45),
...
def plot_monthly_installers_required(schedule_df):
    df = schedule_df[schedule_df['Total_Meters'] > 10].copy()
    df['Year'] = df['Date'].dt.year
    df['Month'] = df['Date'].dt.month
    max_installers_per_subdiv = (
        df.groupby(['Year', 'Month', 'SubDiv'])
        .agg(Max_Installers=('Installers', 'max'))
        .reset_index()
    )
    total_installers_per_month = (
        max_installers_per_subdiv
        .groupby(['Year', 'Month'])
        .agg(Total_Installers_Required=('Max_Installers', 'sum'))
        .reset_index()
    )
    total_installers_per_month['Year_Month'] = total_installers_per_month['Year'].astype(str) + '-' + total_installers_per_month['Month'].astype(str).str.zfill(2)
    total_installers_per_month = total_installers_per_month.sort_values(['Year', 'Month'])
    
    fig = go.Figure()
    fig.add_trace(go.Bar(
        x=total_installers_per_month['Year_Month'],
        y=total_installers_per_month['Total_Installers_Required'],
        marker_color='skyblue',
        text=total_installers_per_month['Total_Installers_Required'].astype(int),
        textposition='auto'
    ))
    
    fig.update_layout(
        title=dict(text='Monthly Installers Required (All Subdivisions)', x=0.5, xanchor='center'),
        xaxis_title='Year-Month',
        yaxis_title='Installers Required',
        template='plotly_white',
        height=600,
        margin=dict(t=80, b=50, l=50, r=50),
        xaxis=dict(tickangle=45),
...
def plot_cumulative_meters_installed(schedule_df):
    df = schedule_df.copy()
    df['Year'] = df['Date'].dt.year
    df['Month'] = df['Date'].dt.month
    monthly = (
    ...
    monthly['Cumulative_Meters_Installed'] = monthly['Meters_Installed'].cumsum()
    
    fig = go.Figure()
    fig.add_trace(go.Scatter(
        x=monthly['Year_Month'],
        y=monthly['Cumulative_Meters_Installed'],
        mode='lines+markers',
        marker=dict(size=8, color='green'),
        line=dict(width=2, color='green'),
        text=monthly['Cumulative_Meters_Installed'].astype(int),
        textposition='top center'
    ))
    
    fig.update_layout(
        title=dict(text='Cumulative Meters Installed Over Time (All Subdivisions)', x=0.5, xanchor='center'),
        xaxis_title='Year-Month',
        yaxis_title='Cumulative Meters Installed',
        template='plotly_white',
        height=600,
        margin=dict(t=80, b=50, l=50, r=50),
        xaxis=dict(tickangle=45),
    ...
    with st.sidebar:
        ...
        with st.expander("📍 Subdivision Selection", expanded=True):
            ...
            select_all = st.checkbox(
                ...
                help="Check to select all subdivisions for aggregate visualizations"
            )
            if select_all:
                ...
                st.session_state.selected_subdivs = st.multiselect(
                    ...
                    help="Select one or more subdivisions for detailed visualizations"
                )
...
def home_page():
    ...
    with st.expander("📖 How to Use This Tool", expanded=True):
        st.markdown("""
        ### Step-by-Step Guide
        1. **Select Subdivisions**: Choose one or more subdivisions in the sidebar. Select 'All Subdivisions' for aggregate visualizations.
        ...
        4. **Visual Analysis**: View automatically displayed charts. Aggregate charts are shown for all subdivisions; detailed charts for individual selections.
        ...
        - **Interactive visualizations**: Automatically displays all relevant charts based on subdivision selection.
...
def visualizations_page():
    ...
    def render_chart(plot_name, fig, description):
        with st.container():
            ...
            with st.expander("💾 Download This Chart"):
                ...
                with col1:
                    ...
                    href = f'<a href="data:image/png;base64,{b64}" download="{plot_name.lower().replace(" ", "_")}.png">⬇️ Download as PNG</a>'
                    st.markdown(href, unsafe_allow_html=True)
                with col2:
                    buf = io.BytesIO()
                    fig.write_image(buf, format="svg", width=1200, height=600)
                    buf.seek(0)
                    b64 = base64.b64encode(buf.read()).decode()
                    href = f'<a href="data:image/svg+xml;base64,{b64}" download="{plot_name.lower().replace(" ", "_")}.svg">⬇️ Download as SVG</a>'
                    st.markdown(href, unsafe_allow_html=True)
            st.markdown("---")
    
    if st.session_state.is_select_all:
        # Aggregate charts for all subdivisions
        render_chart(
            "Monthly Meters Installed (All)",
            plot_monthly_meters_installed(st.session_state.schedule_df),
            "**Total meters installed per month** across all subdivisions."
        )
        render_chart(
            "Monthly Installers Required (All)",
            plot_monthly_installers_required(st.session_state.schedule_df),
            "**Total maximum installers required per month** across all subdivisions."
        )
        render_chart(
            "Cumulative Meters Installed (All)",
            plot_cumulative_meters_installed(st.session_state.schedule_df),
            "**Cumulative meters installed over time** across all subdivisions."
        )
    else:
        ...
        render_chart(
            ...
            plot_installer_requirements(st.session_state.schedule_df, st.session_state.selected_subdivs),
            "**Average and peak installers needed per day** by month for selected subdivisions."
        )
        figs = plot_cumulative_progress(st.session_state.schedule_df, st.session_state.selected_subdivs)
        for i, fig in enumerate(figs):
            title = "Cumulative Meters Installed Over Time" if i == 0 else "Installation Progress Percentage"
            description = "**Progress over time** showing cumulative meters installed and percentage completion for selected subdivisions."
            render_chart(title, fig, description)
